...
const autoRotateState = (() => {
  ...
  if (typeof window !== 'undefined' && typeof window.matchMedia === 'function') {
    try {
      ...
      enabled = (hoverNone && hoverNone.matches) || (pointerCoarse && pointerCoarse.matches);
    } catch (error) {
      enabled = false;
    }
  }
...
function ensureAutoRotateObserver() {
  ...
  if (!autoRotateState.observer) {
    autoRotateState.observer = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          const callbacks = autoRotateState.targets.get(entry.target);
          if (!callbacks) {
            return;
          }
  ...
  return autoRotateState.observer;
}

function registerAutoRotateTarget(target, start, stop) {
  if (!autoRotateState.enabled || !target) {
    return;
  }
  ...
  autoRotateState.targets.set(target, { start, stop });
...
function loadLookbookMetrics() {
  ...
  const metrics = window.LUCE_DEMO?.getMetrics ? window.LUCE_DEMO.getMetrics() : null;
  if (!metrics) {
    return;
  }

  const { values, updated: updatedTarget, error } = metricsState;
  if (values.products) values.products.textContent = formatMetricNumber(metrics.products);
  if (values.suppliers) values.suppliers.textContent = formatMetricNumber(metrics.suppliers);
  if (values.sellers) values.sellers.textContent = formatMetricNumber(metrics.sellers);
  if (values.members) values.members.textContent = formatMetricNumber(metrics.members);
  if (updatedTarget) {
    updatedTarget.textContent = formatMetricDate(metrics.latestProduct);
  }
...
async function reloadLookbookCatalog() {
  ...
  try {
    ...
    const payload = window.LUCE_DEMO?.getProducts ? window.LUCE_DEMO.getProducts() : [];
    lookbookCatalog = Array.isArray(payload) ? payload.map(normalizeProduct) : [];
    lookbookCatalog.sort((a, b) => {
...
function createLookCard(item, sequence = 1) {
  ...
  registerAutoRotateTarget(thumb, startRotation, stopRotation);
...
function initializeLookbook() {
  ...
  if (typeof window !== 'undefined') {
    window.addEventListener('luce:data:changed', () => {
      reloadLookbookCatalog().finally(() => {
        const currentFilter = lookbookState.filter || defaultFilter;
        applyLookbookFilter(currentFilter, setActiveButton);
      });
    });
  }
}
...
function setupSignupForms() {
  ...
  forms.forEach(form => {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        if (!window.LUCE_DEMO || typeof window.LUCE_DEMO.registerAccount !== 'function') {
          throw new Error('데모 데이터 모듈을 찾을 수 없습니다.');
        }

        window.LUCE_DEMO.registerAccount(type, payload);

        const successMessage =
          type === 'supplier'
            ? '공급업체 신청이 접수되었습니다. 담당자가 24시간 내 안내드립니다.'
            : type === 'seller'
            ? '셀러 신청이 접수되었습니다. 곧 온보딩 메일이 발송됩니다.'
            : '일반회원 등록이 완료되었습니다. 소식을 곧 전해드릴게요.';

        feedback.textContent = successMessage;
        feedback.classList.remove('error');
        form.reset();
      } catch (error) {
        feedback.textContent = error.message || '전송에 실패했습니다. 정보를 다시 확인해주세요.';
        feedback.classList.add('error');
      } finally {
        if (submitButton) {
          submitButton.disabled = false;
