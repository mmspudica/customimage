...
const coarsePointerMediaQuery = typeof window !== 'undefined' && typeof window.matchMedia === 'function'
...
function devicePrefersAutoRotation() {
  return Boolean(coarsePointerMediaQuery && coarsePointerMediaQuery.matches);
}

function ensureAutoRotateObserver() {
  if (!devicePrefersAutoRotation()) {
    return null;
  }

  if (!lookbookState.autoRotateObserver) {
    lookbookState.autoRotateHandlers = new Map();
    lookbookState.autoRotateObserver = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          const handler = lookbookState.autoRotateHandlers.get(entry.target);
          if (!handler) {
            return;
          }
  ...
  return lookbookState.autoRotateObserver;
}

function observeAutoRotation(element, start, stop) {
  const observer = ensureAutoRotateObserver();
  if (!observer) {
    return;
  }

  lookbookState.autoRotateHandlers.set(element, { start, stop });
  observer.observe(element);
}

function resetAutoRotationObservers() {
  const { autoRotateObserver, autoRotateHandlers } = lookbookState;
  if (!autoRotateHandlers) {
    return;
  }

  if (autoRotateObserver && autoRotateHandlers.size) {
    autoRotateHandlers.forEach((handler, element) => {
      if (handler && typeof handler.stop === 'function') {
        handler.stop();
      }
      autoRotateObserver.unobserve(element);
    });
  } else if (autoRotateHandlers.size) {
    autoRotateHandlers.forEach(handler => {
      if (handler && typeof handler.stop === 'function') {
        handler.stop();
      }
    });
...
function createLookCard(item, sequence = 1) {
  ...
  observeAutoRotation(thumb, startRotation, stopRotation);
