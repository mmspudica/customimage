...
function stopAllLookRotations() {
  if (!lookbookState.grid) {
    return;
  }

  Array.from(lookbookState.grid.children).forEach(card => {
    if (typeof card._stopRotation === 'function') {
      card._stopRotation();
    }
  });
}

function resetLookbookRotationObserver() {
  stopAllLookRotations();
  if (lookbookState.rotationObserver) {
    lookbookState.rotationObserver.disconnect();
    lookbookState.rotationObserver = null;
  }
}

function ensureRotationObserver() {
  if (!autoRotationState.allowed) {
    return null;
  }

  if (!lookbookState.rotationObserver) {
    lookbookState.rotationObserver = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          const card = entry.target;
          if (entry.isIntersecting) {
            if (typeof card._startRotation === 'function') {
              card._startRotation();
            }
          } else if (typeof card._stopRotation === 'function') {
            card._stopRotation();
          }
        });
      },
  ...
  return lookbookState.rotationObserver;
}

function evaluateAutoRotationPreference() {
  if (!autoRotationState.pointerQuery || !autoRotationState.motionQuery) {
    autoRotationState.allowed = false;
    resetLookbookRotationObserver();
    return;
  }

  autoRotationState.allowed =
    autoRotationState.pointerQuery.matches && !autoRotationState.motionQuery.matches;
...
function initializeAutoRotationPreferences() {
  if (typeof window === 'undefined' || typeof window.matchMedia !== 'function') {
    autoRotationState.allowed = false;
    return;
  }

  autoRotationState.pointerQuery = window.matchMedia('(hover: none)');
  autoRotationState.motionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');

  const handleChange = () => evaluateAutoRotationPreference();

  subscribeMediaQuery(autoRotationState.pointerQuery, handleChange);
  subscribeMediaQuery(autoRotationState.motionQuery, handleChange);

  evaluateAutoRotationPreference();
}
...
function createLookCard(item, sequence = 1) {
  ...
  const startRotation = () => {
    ...
    if (autoRotationState.motionQuery && autoRotationState.motionQuery.matches) {
      return;
    }
    rotationIndex = 1;
  ...
  card._startRotation = startRotation;
  card._stopRotation = stopRotation;
  const rotationObserver = ensureRotationObserver();
  if (rotationObserver) {
    rotationObserver.observe(card);
  }
...
function setupSignupForms() {
  ...
  forms.forEach(form => {
    ...
    form.addEventListener('submit', async event => {
      ...
      const password = typeof entries.password === 'string' ? entries.password : '';
      ...
      Object.entries(entries).forEach(([key, value]) => {
        if (typeof value === 'string' && !value.length && key !== 'password') {
          return;
        }
        payload[key] = value;
      });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initializeAutoRotationPreferences();
  initializeLookbook();
  initializeLookModal();
  initializeSelection();
