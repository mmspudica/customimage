...
const TEST_SUPPLIER_BRAND = '테스트 공급업체';
const TEST_SUPPLIER_PHONE = '010-0000-0000';
const TEST_SUPPLIER_CHANNEL = '테스트 계정';
const TEST_SUPPLIER_EXPERIENCE = 'pilot-ready';
const TEST_SUPPLIER_NOTES = '공급업체 로그인 테스트 계정';
...
async function ensureTestSupplierAccount() {
  ...
  if (existingTest) {
    await run(
      `UPDATE signups
         SET brand = ?, phone = ?, password_hash = ?, password_salt = ?, category = ?, channel = ?, experience = ?, notes = ?
       WHERE id = ?`,
      [
        TEST_SUPPLIER_BRAND,
        TEST_SUPPLIER_PHONE,
        ...
        salt,
        'fashion',
        TEST_SUPPLIER_CHANNEL,
        TEST_SUPPLIER_EXPERIENCE,
        TEST_SUPPLIER_NOTES,
        existingTest.id,
      ]
    );
  ...
  const legacy = await get(
    ...
    ['supplier001@lookbook.app']
  );

  if (legacy) {
    await run(
      `UPDATE signups
         SET email = ?, brand = ?, phone = ?, password_hash = ?, password_salt = ?, category = ?, channel = ?, experience = ?, notes = ?
       WHERE id = ?`,
      [
        TEST_SUPPLIER_EMAIL,
        TEST_SUPPLIER_BRAND,
...
async function seedSuppliers() {
  ...
  await ensureTestSupplierAccount();
  await run('BEGIN TRANSACTION');
  let inserted = 0;

  try {
    for (let i = 1; i < SUPPLIER_TARGET; i += 1) {
      const supplierNumber = i + 1;
      const sequence = String(supplierNumber).padStart(3, '0');
      ...
      const category = categories[(supplierNumber - 1) % categories.length];
      const phone = generatePhone(supplierNumber);
...
async function seedSampleProducts() {
  ...
  for (let i = 0; i < SAMPLE_PRODUCTS.length; i += 1) {
    ...
    const existing = await get(
      `SELECT id FROM products WHERE title = ? AND category = ?`,
    ...
    const primaryImage = gallery[0] || createImageUrl(`${product.title} - Thumb`);
